...
const DeleteModal = ({ open, cloesDelete, dataDelete, loadData }) => {
    ...
    try {
      if (dataDelete.name) {
        notificationContext.show({
          ...
          description: `Phòng ${dataDelete.roomcode} đang có khách ở !`
        })
      } else {
        await api.roomApi.delete({ _id: dataDelete._id });
        cloesDelete()
        loadData()
        notificationContext.show({
          type: "success",
          title: "Đã xóa phòng thành công !",
          // duration: mặt định 4s,
          description: `Phòng ${dataDelete.roomcode} đã được xóa !`
        })
      }
    } catch (err) {
      console.log(err);
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{`Bạn có muốn xóa phòng ${dataDelete.roomcode} Không ?`}</DialogTitle>
